...
def spawn_stethoscopes():
    service_monitors = Service_Monitor.objects.all().values('id')
    g = group(stethoscope.s(sm['id']) for sm in service_monitors)
    g.apply_async()


@shared_task(ignore_result = True)
def stethoscope(sm_id):
    sm = Service_Monitor.objects.get(pk=sm_id)
    try:
    ...
    if service.is_up() and open_issue is None and closed_issue is None:
        # Case 1
        pass
    elif service.is_up() and open_issue is not None and closed_issue is None:
        # Case 2
    ...
    elif service.is_up() and open_issue is None and closed_issue is not None:
        # Case 3
        pass
    elif service.is_up() and open_issue is not None and closed_issue is not None:
        # Case 4
        for incident in open_incidents:
            Event.objects.filter(id=incident['id']).update(
                    status=Status.objects.filter(status='closed').values('id')[0]['id'],
            ...
            cache.delete_many(['timeline','events_ns','event_count_ns'])
    elif not service.is_up() and open_issue is None and closed_issue is None:
        # Case 5
        ...
        cache.delete_many(['timeline','events_ns','event_count_ns'])
    elif not service.is_up() and open_issue is not None and closed_issue is None:
        # Case 6
        if len(open_incidents) > 1:
            for index, incident in enumerate(open_incidents):
                ...
                cache.delete_many(['timeline','events_ns','event_count_ns'])
    elif not service.is_up() and open_issue is None and closed_issue is not None:
        # Case 7
        ...
        cache.delete_many(['timeline','events_ns','event_count_ns'])
    elif not service.is_up() and open_issue is not None and closed_issue is not None:
        # Case 8
        if len(open_incidents) > 1:
            for index, incident in enumerate(open_incidents):
                if index == 0:
